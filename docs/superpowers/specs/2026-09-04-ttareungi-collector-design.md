...
2. `api.fetch_all()` — `1~1000`, `1001~2000`, … 을 순서대로 호출. `list_total_count`는 API마다 의미가 달라 신뢰하지 않는다. **종료 조건은 "받은 행이 page_size보다 적은 페이지를 만나면 멈춘다"이다.** 실시간 API에는 대여소 ID 필터 파라미터가 없어 전량을 받는다 (대여소 약 3천 개 → 실행당 3~4회 호출)
   - 대여소 수가 정확히 `page_size`의 배수면 마지막 페이지 요청에 서울시 API가 `INFO-200`(해당 데이터 없음)을 준다. 이건 실패가 아니라 빈 페이지로 본다 — 그러지 않으면 대여소 수가 그 경계에 머무는 동안 매 실행이 종료 코드 1로 끝난다
3. `transform` — 관심 ID 집합으로 필터, `captured_at` 부여, 행 변환
   - 응답에 실려온 `parkingBikeTotCnt`가 관심 대여소 전부에서 파싱에 실패하면(필드 자체가 없거나, API가 필드명을 바꿨거나) `__main__`이 이 스냅샷을 적재하지 않고 실패로 끝낸다. 개별 대여소가 값을 안 주는 것은 정상이라 전부 실패했을 때만 막는다 — `ignore_duplicates`라 한 번 잘못 적재하면 되돌릴 수 없기 때문이다
4. `store` — `station_snapshot`에 upsert (되돌릴 수 없는 값이라 실패 시 짧게 재시도한다). 응답에 이름·좌표가 함께 오므로 `station` 마스터도 upsert (덮어쓰고 `updated_at` 갱신) — 이쪽은 실패해도 다음 실행에서 다시 채울 수 있으므로 실패해도 전체 실행을 막지 않는다
...
- `RESULT.CODE`가 `INFO-000`이 아니면 실패로 처리. 단 `INFO-200`(해당 데이터 없음)은 예외 — 요청한 범위에 대여소가 없다는 뜻이라 실패가 아니라 빈 페이지로 본다 (§4)
- 타임아웃 10초, 3회 재시도 (1s → 2s → 4s). API 호출뿐 아니라 **`station_snapshot` 적재도 같은 간격으로 재시도한다** — API는 10분 뒤 다시 받을 수 있지만 한 번 놓친 스냅샷은 그럴 수 없어서, 되돌릴 수 없는 쪽이 재시도 없이 한 번 삐끗해 유실되는 일을 막는다. `station` 마스터 적재 실패는 재시도하지 않고 경고만 남긴다 — 다음 실행에서 다시 채울 수 있는 정보라 실패해도 스냅샷 적재나 종료 코드 판단을 막으면 안 된다
...
- **관심 대여소가 있는데 `parking_cnt`가 전부 None이면 실패로 간주하고 적재하지 않는다.** 개별 대여소가 값을 안 주는 것과 달리, 전부 None인 것은 API 응답 형식이 바뀌었거나 `transform.py`의 필드명이 틀렸다는 신호다. 행 수는 정상이라 0행 가드로는 못 잡고, `ignore_duplicates` 때문에 한 번 적재하면 되돌릴 수 없어 아예 적재하지 않는다
